...
int tick = 0;
void dfs2(int u) {          // DFS for normal usage: as graph traversal algorithm
  //printf(" %d", u);                                  // this vertex is visited
  dfs_num[u] = 1;    tick++;          // important step: we mark this vertex as visited
  for (int j = 0; j < (int)AdjListR[u].size(); j++) {
    ii v = AdjListR[u][j];                      // v is a (neighbor, weight) pair
    if (dfs_num[v.first] == 0)                 // important check to avoid cycle
      dfs2(v.first);     // recursively visits unvisited neighbors v of vertex u
  }
  tick++;
  post[u] = tick; 
}
...
int main(){
    ...
    AdjListR.assign(n+1, vii());
    for(int i=0; i<m; i++){
        int u, v; cin>>u>>v;
        AdjList[u].push_back(ii(v, 0));
        AdjListR[v].push_back(ii(u, 0)); 
    }
