...
private:
  vi p, rank;
  vector<ll> setSize;                       // remember: vi is vector<int>
  int numSets;
public:
  UnionFind(int N, vector<ll> r) {  
    setSize.assign(N, 0); for(int i=0; i<N; i++) setSize[i] = r[i]; numSets = N; rank.assign(N, 0);
    p.assign(N, 0); for (int i = 0; i < N; i++) p[i] = i; }
  int findSet(int i) { return (p[i] == i) ? i : (p[i] = findSet(p[i])); }
  bool isSameSet(int i, int j) { return findSet(i) == findSet(j); }
  void unionSet(int i, int j, ll& res) { 
    if (!isSameSet(i, j)) { numSets--; 
    int x = findSet(i), y = findSet(j);
    // rank is used to keep the tree short
    if (rank[x] > rank[y]) { p[y] = x; setSize[x] += setSize[y]; res = max(res, setSize[x]); }  
    else                   { p[x] = y; setSize[y] += setSize[x]; res = max(res, setSize[y]); 
...
void inOrder(int root){
    if(root == -1)
        return;
    if(nodes[root][1] != -1 and nodes[nodes[root][1]][0] >= nodes[root][0])
        p = false;
    
    if(nodes[root][2] != -1 and nodes[nodes[root][2]][0] < nodes[root][0])
        p = false;
          
    inOrder(nodes[root][1]);
    res.push_back(nodes[root][0]); 
    inOrder(nodes[root][2]);    
}
...
int main(){
    ...
    if(is_sorted(res.begin(), res.end()) and p)
        cout<<"CORRECT"<<endl;
    else
        cout<<"INCORRECT"<<endl;     
